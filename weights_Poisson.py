"""
改进2：
    使用泊松分布计算particles的权值
思路：
    当二项分布的重复次数n足够大时可以用泊松分布近似二项分布，而当泊松分布的$\lambda$足够大时，可用高斯分布近似泊松分布。
...
def mouseCallback(event, x, y, flags, null):
    ...
    previous_x = x
    previous_y = y
    PF_pos, _ = estimate(particles, weights)
...
def update(particles, weights, z, R, landmarks):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        # 泊松分布是离散型随机分布，在非整数处取值为0，因此要进行取整
        weights *= scipy.stats.poisson(distance).pmf(round(z[i]))
